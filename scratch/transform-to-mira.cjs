...
async function main() {
  ...
  const css = fs.readFileSync('node_modules/@spartan-ng/cli/src/generators/ui/style-mira.css', 'utf8');
  ...
  console.log(`Starting transformation to Mira style... (dryRun: ${isDryRun})`);
    ...
    for (const templateFile of templateFiles) {
      ...
      if (lib === 'dialog') {
        ...
        if (targetFileName === 'hlm-dialog-content.ts') {
          console.log(`[CUSTOM-UPDATE] ${lib}/${targetFileName} (Applying Mira styles to custom component)`);
          if (!isDryRun) {
            let content = fs.readFileSync(targetFilePath, 'utf8');
            const updated = content.replace(
              /rounded-none p-4 text-xs\/relaxed ring-1 duration-100/,
              'rounded-xl p-4 text-xs/relaxed ring-1 duration-100'
            );
            if (updated !== content) {
              fs.writeFileSync(targetFilePath, updated, 'utf8');
              console.log(`  -> Successfully updated ${targetFilePath} with Mira classes.`);
              modifiedCount++;
            } else {
              console.log(`  -> Notice: Pattern check for ${targetFilePath}`);
            }
          }
          continue;
        }
